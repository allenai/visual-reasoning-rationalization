...
class GPT2VisionAttentiveTransformer(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        if config.n_coordinates:
            self.coordinates_fc = nn.Linear(config.n_coordinates, config.n_embd)

        if config.vision_embd:
            self.visual_features_fc = nn.Linear(config.vision_embd, config.n_embd)
            self.ln_visual_features = LayerNorm(config.n_embd, eps=1e-12)
    ...
    def forward(self,
        ...
        if visual_representations is not None:
            if box_coordinates is not None:
                proj_visual_representations = self.visual_features_fc(visual_representations)
                proj_box_coordinates = self.coordinates_fc(box_coordinates)
                visual_features = self.ln_visual_features(proj_visual_representations + proj_box_coordinates)

                visual_features_flatten = torch.repeat_interleave(visual_features[:, 0, :].unsqueeze(1), repeats=input_ids.shape[1], dim=1)

                for l in range(len(orig_num_situ_or_object_boxes)):
                    n_boxes = orig_num_situ_or_object_boxes.squeeze(-1)[l].item()
                    visual_features_flatten[l, 1:n_boxes, :] = visual_features[l, 1:n_boxes, :]
                hidden_states = inputs_embeds + position_embeds + token_type_embeds + visual_features_flatten
            else:
                inputs_embeds[:, 1:4, :] = visual_representations
                hidden_states = inputs_embeds + position_embeds + token_type_embeds
